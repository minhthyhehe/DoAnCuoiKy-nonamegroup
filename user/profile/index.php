...
if (!empty($_POST)) {
    ...
    if ($id_user && !empty($fullname)) {
      // echo '<pre>$fullname<br />'; var_dump($fullname); echo '</pre>';
      // die();
        //Luu vao database
        $sql = 'update users set fullname = "'.$fullname.'", email = "'.$email.'", username = "'.$username.'", address = "'.$address.'" where id = '.$id_user;
        
        execute($sql);
      
        if (!empty($_FILES)) {
            if (!empty($_FILES['avatar']['name'])) {
                execute('delete from images where type = 4 and id_user = '.$id_user);
            ...
            <div class="signin-right ">
                <form method="post" enctype="multipart/form-data">
                    ...
                    ?>
                        ...
                        <img id="img_avatar" src="../../admin/<?=$avatar?>" alt="" style="max-width: 200px;">
                    </div>
                    <div class="firstname form-control1 form-group">
                        <input type="text"  id="fullname" name="fullname" placeholder="Họ và tên" value="<?=$authen['fullname']?>">
                    </div>
                    <div class="lastname form-control1 form-group">
                        <input type="text" id="address" name="address" placeholder="Địa chỉ" value="<?=$authen['address']?>">
                    </div>
                    ...
                    <div class="email form-control1 form-group">
                        <input type="email" id="email" name="email" placeholder="Email" value="<?=$authen['email']?>">
                    </div>
                    <div class="username form-control1 form-group">
                        <input type="text" id="username" name="username" placeholder="Tên đăng nhập" value="<?=$authen['username']?>">
                    </div>
                    <div class="password form-control1 form-group">
                        <input type="password" id="password" name="password" placeholder="Mật khẩu" value="<?=$authen['password']?>">
                    </div>
                    ...
                    <button class="btn ">Đăng kí</button> -->
                      ...
                      <input class="btn btn-primary btn-user btn-block" type="submit"  value="Lưu">
